...
# Data Science : A short overview

You can download the slides [here](https://github.com/scizmeli/datascience_class_helper_files/raw/master/Introduction%20to%20Data%20Science-ch1%20Slides.pdf).
...
file.remove("some_existing_directory") #Does not run on Windows
```
...
## Saving and reloading workspace contents
...
| *list* | Named lists | list(Name="John",age=12) |
...
## Logical vectors
Logical vectors are usually generated by logical tests (logical operators). For example :
...
```{r}
data.frame(temperature=temp, isgreaterthan13=c1)
temp[c1]
```
...
### Exercise
Create 10 uniformly distributed random integers between [0,100] and store them in a variable called *x*. 
...
Using the result of the previous exercise (variable x), create a factor variable called *myclass* that is of same length of x and that will store the following categories :

- "low" if x is smaller than 30
- "high" if x greater than 70
- "medium" if x is in between

Hint 1 : First create a character vector containing strings ("low", "high" and "medium")
Hint 2 : For more information on factors, consult help topic *factor*.
...
## Data Frames
Data frames (*data.frame*) is the fundamental data type in R. One can think of it as a collection of vectors of equal length put in columns side by sidde to form a table. To

### Creating a data.frame from R variables

```{r}
variable = data.frame(Name=c("John","Alice"), Age=c(12,14), Country=c("Denmark","Turkey"))
```

### Importing csv files as a data.frame
Let us import a customer database from a csv file :
...
```
```{r}
# Read the file contents into a data.frame called "maps"
maps <- read.csv2("MapsThatChangedOurWorld_StoryMap_Data.csv", header=T, sep=";")

# Rename the column 5 to : "Title"
colnames(maps)[5] <- "Title"

# Print to the screen all rows and columns from 1 to 7
maps
```
```{r}
# Search and remove letter "N" from column "Latitude"
maps$Latitude <- gsub("N", "", maps$Latitude)

idx <- grep("W", maps$Longitude)

# Search and remove letter "E" from column "Longitude"
maps$Longitude <- gsub("E", "", maps$Longitude)

# Search and remove letter "W" from column "Longitude"
...
maps$Longitude[idx] <- -maps$Longitude[idx] 
maps
```
```{r}
# Do the same operations for the column "Year"
maps$Year <- gsub("AD", "", maps$Year)
maps$Year <- as.numeric(maps$Year)

```
```{r}
# Print on screen all rows, columns "Longitude","Latitude","Year"
...
Ex : Append a full line of NA into the data.frame "flo"and save the results back into "flo"
```{r}
flo <-tail(rbind(flo, NA))
```


Ex : run apply() again to calculate again the sum of votes of all candidates. What is wrong? How can we fix it?
```{r}
apply(flo[,2:11], 2, sum, na.rm=T)

#or
apply(na.omit(flo[,2:11]),2,sum) 
```
...
Ex : Load into R the "Affairs" dataset distributed by the package "AER"
